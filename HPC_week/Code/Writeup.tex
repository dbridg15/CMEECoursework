...
\begin{document}
	...
	\section*{Neutral Theory Simulations}
		...
		\subsection*{Question 8 - Neutral Time Series}
		 ...
		 Species Richness was plotted for a Neutral Theory simulation with no speciation over 200 generations fig.\ref{fig1}. There was an initial dramatic decrease in species richness in the earlier (first 25) generations and then a more gradual decrease until the species richness eventually reached one. This will always occur where there is no speciation as at each neutral step one individual dies and is replaced by the offspring of another, if no new species are created through speciation then eventually the system will always homogenise to a species richness of one. 
		
			\begin{figure}[H]
				\centering
				\includegraphics[width = \textwidth]{../Results/question_8.pdf}
				\caption{Species richness moving to one when under Neutral Theory simulation with no speciation.}
				\label{fig1}
			\end{figure}
        ...
        Species Richness was plotted for a Neutral Theory simulation with a speciation rate of 0.1 and community size of 100 over 200 generations fig.\ref{fig2}, beginning at both the maximum and minimum richness. Both initial conditions reached the same equilibria species richness. This is because now at each neutral step there are two possible outcomes, either as before one individual dies and is replaced by the identical of another or, there is a chance (the speciation rate) that the dying individual will be replaced by an individual from a new unique species. Given that the chance of these possibilities is identical for individuals in both simulations the dynamic equilibria they reach will be the same. It should be noted that it takes the simulation starting at minimal diversity slightly longer to reach equilibria as at the start of the simulation the chance of species dying out when at maximum diversity is greater than the speciation rate creating new species. \hl{(MAKE THIS BETTER!!!!)} 
		
			\begin{figure}[H]
				\begin{center}
					\centering
				\includegraphics[width = \textwidth]{../Results/question_12.pdf}
				\caption{Species richness under Neutral Theory simulation with 0.1 probability of speciation.}
				\label{fig2}
				\end{center}
			\end{figure}
		...
		A Neutral Theory Simulation with a speciation rate of 0.1 and community size of 100 was run for 2000 generations with an additional 200 'burn-in generations' at the start of the simulation to ensure that the simulated community had reached equilibrium before data was recorded. Species abundances were recorded every 20 generations and grouped in octaves ($2^n$ to $2^n-1$). The average number of species in these octaves is shown below fig.\ref{fig3}. 
		
		As the burn-in generations are not included in the recorded species abundances the initial conditions of the community do not matter. This is because species richness quickly reaches the same equilibrium given minimum or maximum starting richness, as shown above in fig.\ref{fig2}. 
		
		
			\begin{figure}[H]
				\centering
				\includegraphics[width = \textwidth]{../Results/question_16.pdf}
				\caption{Distribution of Species Abundance after a Neutral Theory Simulation with speciation run for 2000 generations.}
				\label{fig3}
			\end{figure}
		...
		\subsection*{Challenge Question A}
	     ...
	     A Neutral Theory Simulation with a speciation rate of 0.1 and community size of 100 was run for 1000 generations, the species richness of the community was recorded each iteration of the simulation. This was repeated 100 times starting both at initial maximum and initial minimum starting richness. The mean species richness was plotted fig.\ref{ca}. From this it is clear that the community reached dynamic equilibrium by approximately generation 50, and therefore a much shorter burn in period could have been used in question 16.
		
		
			\begin{figure}[H]
				\centering
				\includegraphics[width = \textwidth]{../Results/challenge_A.pdf}
				\caption{Average species richness from 100 repeated simulations for the first 1000 generations. Community size of 100 starting at both maximum and minimum initial richness, 97.2\% confidence intervals shown. Vertical line at generation 50 - approximately where dynamic equilibrium is reached.}
				\label{ca}
			\end{figure}
		...
		%no need for any writing!!
			...
			\begin{figure}[H]
				...
				\caption{Average species richness from 25 repeated simulations for first 500 generation. Community size of 100 with 11 different starting initial richness', 97.2\% confidence intervals shown.}
				\label{cb}
			\end{figure}
	...
	\section*{Fractals in Nature}
		...
		The fractal dimensions of the fist image shown (Sierpinski Carpet) is $(\log{(8)}/\log{(3)})$ which is approximately 1.893. This is calculated as the shape has a width of 3 and a size of 8 (8 of the original shape are required to assemble a new level of the fractal). Given that $width = size^{dimension}$, the dimension can therefore be calculated by rearranging to $dimension = \log{(size)}/\log{(width)}$.
        ...
        This Sierpinski Triangle was drawn between specific equidistant points to ensure it was an equilateral triangle fig.\ref{fig7}. The initial point was started outside of the triangle (coloured red in the top left corner), the first 20 points are also coloured red and if you look  closely you can see that a few points lie within the white space but once one is plotted in the Sierpinski Triangle then all following points will also. 
	   ...
	   When attempting to draw a spiral a recursive function was used in R. This function called itself to continually draw lines $\pi/4$ radians to the right and 0.95 the previous length. This produces a stack overflow error as the computer does not stop drawing lines and continually attempts to add smaller and smaller lines Ad infinitum.
	   ...
	   To overcome this problem an 'if statement' was put within the function so new lines were not drawn below a threshold length. This prevented the computer running out of memory and running into an error allowing fig.\ref{fig8} to be drawn. 
		...
		\subsection*{Question 24 - Tree}
		 ...
		 Similar to the function spiral function, the tree function calls itself but this time twice, once with a direction of $pi/4$ radians to the right and once the same to the left. The length is also decreased more each iteration, this time 0.65 times. The function produces the below fractal tree fig.\ref{fig9}.
		...
		\subsection*{Challenge Question F}
	        ...
	        Changing the minimum threshold length below which the computer stopped adding branches had a significant impact on the time it took for the fern\_2 function to run. With an initial length of 1 it took my (not particularly powerful) computer 0.169s to draw a fern when this value was set to 0.1, changing this value to 0.005 had a significant impact on the time taken - now 23.599s. This is due to the fact that with each new iteration the computer has to draw exponentially more lines - two for each previous line it has drawn.  
            ...
            I also attempted to write a function for a Koch Snowflake to put at the top of the tree as a star (challenge\_F1.1), I wasn't completely successful, but you can see I was starting to get close fig.\ref{fig12}.
					...
					\begin{figure}[H]
						...
						\caption{Closer look at attemp	ted Koch Snowflake.}
						\label{fig12}	
					\end{figure}
