"""
    ...
    Needs: Adjacency list of who eats whom (consumer name/id in 1st column,
    resource name/id in 2nd column. and list of species names/ids and properties
    such as biomass (node abundance), or average body mass.
...
Sps = sc.unique(AdjL)  # get the species ids
Sizs = sc.random.uniform(SizRan[0], SizRan[1], MaxN)  # Generate body size
...
NodSizs = 10**-32 + (Sizs-min(Sizs))/(max(Sizs)-min(Sizs))  # node sizes
